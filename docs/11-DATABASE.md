...
运行时数据库为 PostgreSQL，目前包含三张业务表：

- `indicators`：市场指标当前值及元数据。
- `macro_events`：宏观事件、前值、预期和实际值。
- `daily_market_notes`：按日期保存交易主线、证据和明日观察。
...
每天保存一份独立的 Trading Journal。行情快照继续读取`indicators`，日志只保存交易员判断、所选指标代码、证据备注和最多三条明日观察，不复制行情值。

### 约束与索引

- 主键：`id` identity。
- 唯一约束：`note_date`，保证一日一条并支持幂等 upsert。
- `thesis`只允许产品定义的九个选项。
- `summary`长度为1–200字。
- 三个 JSONB 字段必须为 array，`watchlist`最多3项。
- `daily_market_notes_updated_at_idx`支持按最近更新时间审计。
- 证据 symbol 由 API 校验存在性；v0.1不建立 JSONB 到 indicators 的外键。

迁移文件：`sql/002_daily_market_notes.sql`。应用启动不会自动执行迁移。

Staging 使用受环境、确认值和 Supabase 项目标识保护的命令执行：`npm run migrate:staging:journal`。该命令不允许用于 Production。

## Sequence / Identity

三张表的 `id` 均使用 `GENERATED BY DEFAULT AS IDENTITY`。PostgreSQL 为 identity 列维护 sequence；常规 INSERT 不应显式传入 ID。
