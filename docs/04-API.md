...
| GET | `/api/journal/:date` | Bearer Token + path date | 200 note / 401 | 读取现阶段 Journal；用户隔离将在 v0.3 完成 |
| PUT | `/api/journal/:date` | Bearer Token + path date + JSON | 200 note / 400 / 401 | 更新现阶段 Journal；用户隔离将在 v0.3 完成 |
