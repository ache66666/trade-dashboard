...
| GET | `/api/journal/:date` | Bearer Token + path date | 200 note / 401 / 503 | Node 验证 Token 后通过 Supabase Data API 读取；用户隔离将在 v0.3 完成 |
| PUT | `/api/journal/:date` | Bearer Token + path date + JSON | 200 note / 400 / 401 / 503 | Node 验证 Token 后通过 Supabase Data API 更新；用户隔离将在 v0.3 完成 |
...
要求有效 Bearer Token，读取指定 ISO 日期的交易日志。无记录时返回200和`note:null`：
...
要求有效 Bearer Token，按日期幂等新增或更新交易日志：
