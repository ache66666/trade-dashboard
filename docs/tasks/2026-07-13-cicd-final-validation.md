...
- Seed使用`STG_*`和`STAGING SEED`专用标识，双重环境确认。
...
- 数据库影响：schema无变更；仅显式执行时向Staging加入5/2测试数据。
- 环境变量影响：支持Commit/版本/部署时间；Seed新增显式确认值。
...
| 风险 | 缓解措施 |
| --- | --- |
| Seed误连Production | APP_ENV与确认值双重检查，数据库模块加载前拒绝 |
