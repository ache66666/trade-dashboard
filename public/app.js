...
(function () {
  ...
  function renderAuthState(state) {
    ...
    var journalNav = byId('journalNav');
    ...
    if (state.authenticated) {
      ...
      sessionPanel.hidden = true;
      journalNav.hidden = true;
      if (byId('journal').className.indexOf('active') >= 0) showPage('overview');
  ...
  function showPage(page) {
    if (page === 'journal' && (!authClient || !authClient.getSession())) {
