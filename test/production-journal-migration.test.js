...
const productionUrl = `postgresql://postgres.${productionRef}@pooler.example:6543/postgres`;
...
function constraints(target) {
  const result = [
    ...
    { name:'daily_market_notes_thesis_check', type:'CHECK', definition:'CHECK (true)' },
    { name:'daily_market_notes_summary_length_check', type:'CHECK', definition:'CHECK (true)' },
    { name:'daily_market_notes_supporting_array_check', type:'CHECK', definition:'CHECK (true)' },
    { name:'daily_market_notes_opposing_array_check', type:'CHECK', definition:'CHECK (true)' },
    { name:'daily_market_notes_watchlist_array_check', type:'CHECK', definition:'CHECK (true)' }
  ];
  if (target) {
...
test('anon grants and missing authenticated grants are rejected', () => {
  ...
  assert.ok(result.mismatches.includes('grant:anon-select'));
...
test('formal execution applies SQL once and commits only after target verification', async () => {
  ...
  assert.deepEqual(queries, ['BEGIN', 'SELECT 1 AS migration_body', 'COMMIT']);
});
...
test('CLI sanitizes unexpected database and network errors', () => {
  ...
  assert.equal(sanitized, 'Production Journal migration failed. No database changes were committed.');
