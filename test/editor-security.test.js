...
test('an explicitly enabled Staging server preserves the existing write path', () => {
  const script = [
    ...
    "const response=await fetch('http://127.0.0.1:'+address.port+'/api/indicators',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({symbol:'TEST',name:'Test',category:'Rates',value:1,previous_value:1,source:'Test',as_of:'2026-07-15',frequency:'Daily',change_type:'percent'})})",
...
test('Production HTML hides anonymous Editor controls', async () => {
  ...
  assert.doesNotMatch(response.text, /__EDITOR_WRITE_(?:ENABLED|HIDDEN)__/);
});
